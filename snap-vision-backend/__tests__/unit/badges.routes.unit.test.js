...
jest.mock("../../src/services/badgeService", () => ({
  ...
  completeChallengeForUser:    jest.fn().mockResolvedValue({ points: 67, completedChallenges: ["c1"] }),
...
describe("Badges routes (unit)", () => {
  ...
  it("POST /challenges/complete → 200 and returns completeChallengeForUser payload", async () => {
    ...
    const res = await request(app).post("/api/badges/challenges/complete").send(payload).expect(200);
  ...
  it("POST /increment-routes → 200 and returns incrementRoutesCompletedForUser payload", async () => {
    const res = await request(app).post("/api/badges/increment-routes").send({ uid: "u5" }).expect(200);
