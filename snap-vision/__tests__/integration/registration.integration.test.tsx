...
describe('Registration Integration Tests', () => {
    ...
    it('shows error if email already in use', async () => {
        ...
        fireEvent.press(getByTestId('register-button'));
        await waitFor(() => {
            expect(Alert.alert).toHaveBeenCalledWith('Registration Error', 'This email is already registered.');
        });
    });
    ...
    it('registers user with valid credentials', async () => {
        ...
        fireEvent.press(getByTestId('register-button'));
        await waitFor(() => {
            expect(mockCreateUserWithEmailAndPassword).toHaveBeenCalledWith('test@example.com', 'Password1!');
