...
describe('badgeService (integration)', () => {
  ...
  it('completeChallengeForUser adds challenge and points', async () => {
    ...
    const updated = await badgeService.completeChallengeForUser(testUserId, 'challenge-1');
  ...
  it('incrementRoutesCompletedForUser unlocks route badge', async () => {
    // simulate 9 routes
    await db.collection('users').doc(testUserId).update({ routesCompleted: 9, badges: [] });

    const updated = await badgeService.incrementRoutesCompletedForUser(testUserId);
