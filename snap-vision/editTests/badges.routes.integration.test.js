...
describe('Badges API routes (integration)', () => {
  it('POST /api/badges/unlock → 200 + updated data', async () => {
    const res = await request(server).post('/api/badges/unlock').send({ uid, badgeId }).expect(200);
  ...
  it('POST /api/badges/purchase → 200 + new points', async () => {
    ...
    const res = await request(server).post('/api/badges/purchase').send({ uid, item }).expect(200);
