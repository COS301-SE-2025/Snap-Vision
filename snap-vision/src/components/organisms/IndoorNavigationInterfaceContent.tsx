  ...
  try {
    ...
    if (roomsData.length === 0) {
      ...
      try {
        ...
        console.log('[IndoorNav] buildingName query failed or no index, will fallback to client filter');
      }
    }
    ...
    if (roomsData.length === 0) {
      ...
      roomsData = allRooms.filter((r: any) =>
        r.buildingId === buildingId || r.buildingName === buildingName,
      );
    }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.roomItem,
                  {
                    backgroundColor: selectedStartRoom?.id === item.id ? colors.primary : colors.card,
              ...
              >
                <View style={styles.roomHeader}>
                  <Text style={[styles.roomName, { color: selectedStartRoom?.id === item.id ? '#FFF' : colors.text }]}>{item.name}</Text>
                  <Text style={[styles.roomFloor, { color: selectedStartRoom?.id === item.id ? '#FFF' : colors.secondary }]}>{item.floorId}</Text>
                </View>
                {!!item.type && <Text style={[styles.roomType, { color: selectedStartRoom?.id === item.id ? '#FFF' : colors.secondary }]}>{item.type}</Text>}
                {!!item.description && <Text style={[styles.roomDescription, { color: selectedStartRoom?.id === item.id ? '#FFF' : colors.secondary }]}>{item.description}</Text>}
              </TouchableOpacity>
            )}
          />
        </View>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  <View style={styles.roomHeader}>
                    <Text style={[styles.roomName, { color: selected ? '#FFF' : colors.text }]}>{item.name}</Text>
                    <Text style={[styles.roomFloor, { color: selected ? '#FFF' : colors.secondary }]}>{item.floorId}</Text>
                  </View>
                  {!!item.type && <Text style={[styles.roomType, { color: selected ? '#FFF' : colors.secondary }]}>{item.type}</Text>}
                  {!!item.description && <Text style={[styles.roomDescription, { color: selected ? '#FFF' : colors.secondary }]}>{item.description}</Text>}
                </TouchableOpacity>
              );
            }}
          />
      ...
      <TouchableOpacity
        style={[
          ...
          { backgroundColor: selectedStartRoom && selectedEndRoom ? colors.primary : colors.secondary, opacity: selectedStartRoom && selectedEndRoom ? 1 : 0.5 },
        ]}
...
const styles = StyleSheet.create({
  ...
  noticeContainer: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 8, padding: 12, borderRadius: 8, gap: 8 },
