...
function parseIBeaconBytes(bytes: number[]): { uuid: string; major: number; minor: number } | null {
  ...
  let idx = -1;
  for (let i = 0; i < bytes.length - 1; i++) if (bytes[i] === 0x02 && bytes[i + 1] === 0x15) { idx = i; break; }
...
export class NativeBeaconScanner implements BeaconScanner {
  ...
  async start(onBatch:(readings:IBeaconReading[])=>void) {
    if (this.running) { log('start: already running'); return; }
    this.running = true; this.seenCount = 0; this.buffer = [];
    ...
    try { await BleManager.checkState(); } catch {}
  ...
  async stop() {
    if (!this.running) { log('stop: not running'); return; }
    this.running = false;
    log('stop: stopping…');
    try { await BleManager.stopScan(); } catch {}

    this.subDiscover?.remove?.(); this.subDiscover = undefined;
    this.subStop?.remove?.();     this.subStop     = undefined;
    ...
    const dropped = this.buffer.length; this.buffer = [];
