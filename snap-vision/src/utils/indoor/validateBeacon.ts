...
export function validateBeacon(input: BeaconInput) {
  ...
  if (!UUID_RE.test(uuid)) errors.push('UUID must be a valid v1–v5 UUID (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx).');
  ...
  if (!Number.isInteger(major) || major < 0 || major > 65535) errors.push('Major must be an integer 0–65535.');
  if (!Number.isInteger(minor) || minor < 0 || minor > 65535) errors.push('Minor must be an integer 0–65535.');
  ...
  const tx = input.txPowerAt1m === undefined || input.txPowerAt1m === '' ? -59 : Number(input.txPowerAt1m);
  if (!Number.isFinite(tx) || tx < -100 || tx > -30) errors.push('txPowerAt1m must be between -100 and -30 dBm (or leave blank).');
